...
Consecutive Points (**team_one_most_consecutive_points/team_two_most_consecutive_**
**points**, int): A quantitative variable that corresponds to the greatest amount of consecutive points a player scores during the three games, with consecutive points carried from game to game. An extreme value would be observed when a player scores more than 8 consecutive points. The extreme maxima that can be scored is 42 consecutive points, while the extreme minima is 0.
...
### Response Variables
1. Winner (**winner**, int): the winner of the match (either 0, 1, or 2). An integer of 1 designates the winner of the match as team 1, while an integer of 2 designates the winner of the match as team 2. An integer of 0 means that one of the players retired and thus cannot continue the match, due to extraneous circumstances like injury. This variable does not have any extreme values.

2. Total Points (**team_one_total_points/team_two_total_points**, int): A quantitative variable that describes the number of points earned by each player for the entire match. An extreme value in the data set would be a player that scores over 63 points in a single match. The extreme maxima that can be reached is 89 points, whereas the extreme minima is 0.
...
The next code chunks shows how we're loading in the second data set with the players and their ranking for each week from 2018 to 2021. The dates were previously listed as column names, but we reorganized the data set so that the dates would be values under the new column "Date". The values under the dates columns are moved to a new column named "rank". Since the first and last names of our players were separated into two columns, we combined the two columns so that just one column would represent the full name of the player.   
...
The problem with the data set is that values under 'ranking_matters' is saved as a string, not a numeric value. So, we will change our data type of our 'ranking_matters' column from characters to numeric values because player ranks can be categorized as a quantitative variable. 
...
We now have 2 tables with data of Team 1 or Team 2 winning with each team's ranks. Both tables have a 1 if the higher-ranked team won, and a 0 in the lower-ranked team won We can combine these tables and take the mean of the 'ranking_matters' column.
The result is the probability that the team the higher-ranked player/team will win a match based on the Men's Single data. 
...
The results show that 67.3% of teams that won their match were ranked higher than their opponent. Let's check to see if 67.3% is a good enough probability to prove whether or not ranking is a true factor that affects the probability of winning. In other words, we're checking to see if our prediction of 67.3% of players ranked higher winning their match is purely by chance because of the data set we're using, or if our prediction of 67.3% is random and applicable in any scenario. To do so, we'll be performing a t-test:
...
# Opponent's by at least a certain value from 10 to 200 in increments of 5.
...
What our above graph shows is that, as the rank gap between teams increases from 10 to our proposed 200 rank gap, the chances of the higher-ranked team winning increases as well. This supports our overall hypothesis that the higher-ranked team wins more often than the lower-ranked team. We can see that although the graph follows what appears to be an exponential trend, there is an unusual growth/discrepancy in win percentage when the difference in rankings is around 10 to 30 before dipping back down. We believe that this is due to the uncertainty that comes with having matches that involve two players of very similar rankings. Because it is difficult to tell who has a higher chance of winning given such similar rankings, when the data includes the matches with low ranking differences, it fluctuates more; thus, resulting in the unusual hump in the graph.
...
p <- fixed_match_winner_men %>% 
  mutate(set_won_and_winner = ifelse(winner == game_1_winner, 
                                     "won_first_set_and_won_match", 
                                     "won_second_set_and_won_match")) %>% 
...
Our results show that, if you win the first game, you have win the second game 64.6% of the time. 
...
First, we need to source our country code from an outside source to match our tournament location with our team's nationality. We do so by pulling country code data from Wikipedia. We cross-referenced gthe country code from Wikipedia with the official country codes per country to ensure that the codes we're sourcing from Wikipedia are accurate. 
...
We can now tell from the side-by-side bar chart above that, for every country, our players' win rates in their home country were higher than their win rates outside of their home country, therefore confirming our hypothesis that homecourt advantage plays a role in increasing the match win rate. 
